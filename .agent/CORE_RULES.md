# Global Agent Engineering Standards (CORE\_RULES)
...
## 8. Code Quality Gates (CI enforcement)

Every file written or modified MUST pass these checks locally before
the change is considered complete. The agent must validate these —
never assume the code is clean.

### Python files

* **Black**: code must be Black-formatted. Run mentally: would `black --check`
  pass on this file? Line length 88, double quotes, standard Black rules.
  * Never write lines that Black would reformat (long chained calls,
    inconsistent quotes, trailing commas missing in multi-line structures)
* **Ruff**: no lint violations. Common ones to avoid:
  * unused imports
  * undefined variables
  * f-strings without placeholders
  * bare `except:` clauses
* **Mypy**: no type errors. Every function MUST have type annotations.
  Return types, parameter types — no bare `def fn(x):` allowed.
* **Pre-commit hooks**: treat pre-commit checks as a hard gate.
  If a pre-commit hook would fail, the code is not done.

### Node.js files

* **ESLint**: no lint errors. Warnings are acceptable, errors are not.
* Write code that passes the project's existing ESLint config —
  do not introduce rules violations.

### Universal

* Never leave debug `print()` or `console.log()` statements in committed code
* Every new function needs at least a one-line docstring (Python)
  or JSDoc comment (JS/TS)

## 9. Security Layer — Mandatory Before Any Data Touches the System

* **Rule:** Every endpoint, DB write, and LLM call involving candidate data
  must pass the security checklist in `.agent/security_checklist.md` before
  the change is considered complete.
* **Why:** The platform handles candidate PII — emails, transcripts, voice
  recordings. A single unguarded endpoint is a GDPR violation and a trust
  catastrophe.
* **Enforcement:**
  * Every new API endpoint MUST have auth middleware applied — no exceptions
  * All DB queries MUST use parameterized queries — never string concatenation
  * Candidate PII MUST be stripped before sending to any external API (LLM, STT, TTS)
  * `.env` MUST be in `.gitignore` — verify before every commit
  * API keys MUST never appear in any log, response body, or error message
  * Run `.agent/security_checklist.md` on every High-risk change

## 10. Context Budget — Prevent Context Rot

* **Rule:** The agent must never load more context than necessary per prompt.
* **Enforcement:**
  * Max 2 skill files loaded per prompt
  * `issue_log.md` — reference last 10 entries only; archive older entries
    monthly to `.agent/archive/issue_log_YYYY-MM.md`
  * `audit_log.md` — reference last 5 entries only; same archive rule
  * For single-file low-risk changes: skip doc\_sync until fix is confirmed stable
  * Token overhead target: keep `.agent/` file loading under 5,000 tokens total
