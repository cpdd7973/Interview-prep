...
async def evaluator_node(state: InterviewState) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"📊 Evaluator Agent processing interview for {state.get('candidate_name')}"
    )
    ...
    for msg in messages:
        if isinstance(msg, SystemMessage):
            ...
            formatted_transcript.append(
                {"speaker": "CANDIDATE", "content": msg.content}
            )
        else:
    ...
    if asked_ids:
        ...
        resp = question_bank_mcp.get_questions_by_role(
            GetQuestionsInput(role=state["job_role"], limit=20)
        )
        if resp.get("success"):
    ...
    eval_resp = await evaluator_mcp.calculate_dimension_scores(
        CalculateDimensionScoresInput(
    ...
    try:
        ...
        if room_id:
            db_eval = Evaluation(
                ...
                communication_score=evaluation_data["scores"].get(
                    "communication_score", 0
                ),
                problem_solving_score=evaluation_data["scores"].get(
            ...
            existing = (
                db.query(Evaluation).filter(Evaluation.room_id == room_id).first()
            )
            if existing:
                db.delete(existing)
            db.add(db_eval)
    ...
    # Update State
    return {"status": "EVALUATED", "evaluation": evaluation_data}
