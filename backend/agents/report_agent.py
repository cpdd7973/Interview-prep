...
def report_node(state: InterviewState) -> Dict[str, Any]:
    ...
    try:
        ...
        compile_resp = report_mcp.compile_report(
            CompileReportInput(
                session_data=session_data, evaluation_data=state["evaluation"]
            )
        )
        ...
        export_resp = report_mcp.export_pdf(
            ExportPdfInput(report_data=report_data, output_path=pdf_path)
        )
        ...
        try:
            ...
            if evaluation:
                ...
                db.commit()
                logger.info("💾 Saved report_path to Evaluation record")
