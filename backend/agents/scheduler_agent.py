...
def schedule_interview_node(state: InterviewState) -> Dict[str, Any]:
    ...
    try:
        ...
        room_resp = room_mcp.create_daily_room(
            CreateRoomInput(
                room_id=room_id,
                duration_minutes=settings.max_interview_duration_minutes
                + settings.early_entry_minutes,
            )
        )
        ...
        if calendar_mcp._has_oauth2_credentials():
            cal_resp = calendar_mcp.create_event(
                CreateEventInput(
                    summary=f"Interview: {candidate_name} - {job_role}",
                    start_time=scheduled_at,
                    end_time=scheduled_at
                    + timedelta(minutes=settings.max_interview_duration_minutes),
            ...
            if cal_resp.get("success"):
                logger.info(
                    f"✅ Created calendar event via {cal_resp.get('method', 'unknown')}"
                )
            else:
                logger.warning(
                    f"⚠️ Calendar event creation failed: {cal_resp.get('error')}. Continuing without it."
                )
        else:
            logger.info("⏭️ Skipping calendar event (no OAuth2 credentials configured)")

        # 5. Create Session in Database
        sess_resp = session_mcp.create_session(
            CreateSessionInput(
        ...
        def _send_email_bg():
            try:
                gmail_resp = gmail_mcp.send_email(
                    SendEmailInput(
                ...
                if gmail_resp.get("success"):
                    logger.info(
                        f"✅ Sent invitation email via {gmail_resp.get('method', 'unknown')}"
                    )
                else:
                    logger.warning(
                        f"⚠️ Invitation email failed: {gmail_resp.get('error')}"
                    )
        ...
        return {
            ...
            "messages": [
                SystemMessage(
                    content=f"Successfully scheduled interview for {candidate_name} at {scheduled_at}. Room: {frontend_link}"
                )
            ],
        }
