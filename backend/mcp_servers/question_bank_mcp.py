...
class SemanticSearchInput(BaseModel):
    query: str = Field(
        ..., description="The context or answer to search related questions for"
    )
...
class QuestionBankServer:
    ...
    def initialize(self):
        ...
        if self._initialized:
            return
        logger.info(
            "Starting background synchronization of Question Bank to ChromaDB..."
        )
    ...
    def _get_collection(self, role: str):
        collection_name = (
            f"{role}_questions".replace("-", "_").replace(" ", "_").lower()
        )
    ...
    def _sync_question_to_chroma(self, question: Question):
        ...
        answer_text = (
            ...
            else "No specific answer provided."
        )
            ...
            metadatas=[
                {
                    ...
                    "ideal_answer": answer_text[
                        :4000
                    ],  # Limiting size for Chroma metadata constraints
                }
            ],
            ids=[str(question.id)],
        )
    ...
    def _load_initial_data(self):
        ...
        try:
            ...
            if count > 0:
                ...
                logger.info(
                    f"Loaded {len(questions)} questions into ChromaDB from SQLite"
                )
                return
            ...
            for filename in os.listdir(q_bank_dir):
                if filename.endswith(".json"):
                    ...
                    self.bulk_import_questions(
                        BulkImportInput(json_file_path=file_path)
                    )
    ...
    def get_questions_by_role(self, input_data: GetQuestionsInput) -> Dict[str, Any]:
        ...
        try:
            ...
            if not questions and input_data.role.strip().lower() != "software_engineer":
                logger.warning(
                    f"No questions found for role '{input_data.role}'. Falling back to 'software_engineer'."
                )
                fallback_query = db.query(Question).filter(
                    Question.role == "software_engineer"
                )
                if input_data.difficulty:
                    fallback_query = fallback_query.filter(
                        Question.difficulty == input_data.difficulty
                    )
                questions = (
                    fallback_query.order_by(func.random()).limit(input_data.limit).all()
                )
    ...
    def edit_question(self, input_data: EditQuestionInput) -> Dict[str, Any]:
        ...
        try:
            question = (
                db.query(Question).filter(Question.id == input_data.question_id).first()
            )
            if not question:
    ...
    def bulk_import_questions(self, input_data: BulkImportInput) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "imported_count": imported_count, "role": role}
        except Exception as e:
            db.rollback()
