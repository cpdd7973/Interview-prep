...
class SendEmailInput(BaseModel):
    ...
    attachment_path: Optional[str] = Field(
        None, description="Path to file attachment (e.g. PDF)"
    )
...
class CheckScheduleInput(BaseModel):
    date_str: str = Field(
        ..., description="Date to check schedule in YYYY-MM-DD format"
    )
...
class GmailMCPServer:
    ...
    def _has_oauth2_credentials(self) -> bool:
        return bool(
            ...
            and settings.google_refresh_token != "your_refresh_token"  # nosec B105
        )
    ...
    def send_email(self, input_data: SendEmailInput) -> Dict[str, Any]:
        ...
        if self._has_smtp_credentials():
            ...
            if result["success"]:
                return result
            logger.warning("⚠️ SMTP failed, trying GWS CLI...")
    ...
    def _send_via_smtp(self, input_data: SendEmailInput) -> Dict[str, Any]:
        ...
        try:
            ...
            # Attachment (PDF)
            if input_data.attachment_path and os.path.exists(
                input_data.attachment_path
            ):
    ...
    def _send_via_oauth2(self, input_data: SendEmailInput) -> Dict[str, Any]:
        ...
        try:
            ...
            msg = EmailMessage()
            msg.set_content(
                input_data.body, subtype="html" if input_data.is_html else "plain"
            )
            msg["To"] = input_data.to_email
    ...
    def read_inbox(self, input_data: ReadInboxInput) -> Dict[str, Any]:
        ...
        return {
            "success": False,
            "error": "No inbox method available. Run 'gws auth login' or set OAuth2 credentials.",
        }
    ...
    def _read_inbox_via_gws(self, input_data: ReadInboxInput) -> Dict[str, Any]:
        ...
        try:
            ...
            return gws_read_inbox(
                query=input_data.query, max_results=input_data.max_results
            )
        except Exception as e:
    ...
    def _check_schedule_via_oauth2(
        self, input_data: CheckScheduleInput
    ) -> Dict[str, Any]:
