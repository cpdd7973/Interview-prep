...
class CreateSessionInput(BaseModel):
    ...
    scheduled_at: datetime = Field(
        ..., description="Scheduled interview datetime (UTC)"
    )
...
class LogTranscriptInput(BaseModel):
    ...
    question_id: Optional[int] = Field(
        None, description="Related question ID if applicable"
    )
...
class SessionMCPServer:
    ...
    def create_session(self, input_data: CreateSessionInput) -> Dict[str, Any]:
        ...
        try:
            ...
            candidate = (
                db.query(Candidate)
                .filter(Candidate.email == input_data.candidate_email)
                .first()
            )
            if not candidate:
                candidate = Candidate(
                    email=input_data.candidate_email, name=input_data.candidate_name
                )
                db.add(candidate)
                db.flush()  # get ID
    ...
    def get_session(self, room_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "scheduled_at": session.scheduled_at.isoformat() + "Z",
                "activated_at": (
                    session.activated_at.isoformat() + "Z"
                ...
                "finished_at": (
                    session.finished_at.isoformat() + "Z"
    ...
    def update_status(self, input_data: Any) -> Dict[str, Any]:
        ...
        room_id = (
            input_data.get("room_id")
            if isinstance(input_data, dict)
            else input_data.room_id
        )
        status = (
        ...
        try:
            ...
            logger.info(
                f"✅ Session {room_id} status: {old_status.value} → {status if isinstance(status, str) else status.value}"
            )
    ...
    def arm_scheduler_job(self, room_id: str, scheduled_at: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                return {"success": False, "error": "Failed to arm activation job"}
    ...
    def cancel_scheduler_job(self, room_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                return {"success": False, "error": "Failed to cancel scheduler job"}
    ...
    def log_transcript_chunk(self, input_data: LogTranscriptInput) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"📝 Logged transcript chunk {chunk.id} for {input_data.room_id}"
            )
    ...
    def list_sessions(
        ...
        try:
            ...
            sessions = (
                query.order_by(InterviewSession.scheduled_at.desc()).limit(limit).all()
            )
            ...
            session_list = [
                {
                    ...
                    "disconnected_at": (
                        s.disconnected_at.isoformat() + "Z"
                        if s.disconnected_at
                        else None
                    ),
                    "finished_at": (
                        s.finished_at.isoformat() + "Z" if s.finished_at else None
                    ),
                    "report_generated_at": (
                        s.report_generated_at.isoformat() + "Z"
