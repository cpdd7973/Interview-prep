...
class CalculateDimensionScoresInput(BaseModel):
    ...
    question_bank: List[Dict[str, Any]] = Field(
        ..., description="Questions asked with ideal answers"
    )
...
Candidate Answer: {input_data.answer}"""
    ...
    async def calculate_dimension_scores(
        self, input_data: CalculateDimensionScoresInput
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Evaluating entire transcript for {input_data.candidate_name}..."
            )
            # Use async LLM invocation
            response_text = await llm_client.invoke_async(
