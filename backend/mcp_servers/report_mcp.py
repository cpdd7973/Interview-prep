...
class ReportMCPServer:
    ...
    def export_pdf(self, input_data: ExportPdfInput) -> Dict[str, Any]:
        try:
            ...
            elements.append(
                Paragraph(
                    f"Generated: {generated[:19].replace('T', ' ')} UTC", subtitle_style
                )
            )
            ...
            score_rows = [
                ...
                [
                    ...
                    f"{tech:.1f} / 10",
                    self._get_verdict_label(tech),
                ],
                [
                    "Communication Clarity (20%)",
                    f"{comm:.1f} / 10",
                    self._get_verdict_label(comm),
                ],
                [
                    "Problem Solving (25%)",
                    f"{prob:.1f} / 10",
                    self._get_verdict_label(prob),
                ],
                [
                    "Behavioral Fit (15%)",
                    f"{behav:.1f} / 10",
                    self._get_verdict_label(behav),
                ],
                [
                    "Confidence & Completeness (10%)",
                    f"{conf:.1f} / 10",
                    self._get_verdict_label(conf),
                ],
            ]
            ...
            for i, s in enumerate(all_scores):
                table_style.append(
                    ("TEXTCOLOR", (2, i + 1), (2, i + 1), self._get_score_color(s))
                )
                table_style.append(
            ...
            elements.append(
                Paragraph(
                    "Generated by Interview Agent System • Confidential", footer_style
                )
            )
            ...
            return {"success": True, "pdf_path": input_data.output_path}
        except Exception as e:
