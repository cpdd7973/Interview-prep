...
async def interview_websocket(websocket: WebSocket, room_id: str):
    ...
    try:
        ...
        if chat_state["messages"]:
            logger.info(f"Session {room_id} already has {len(chat_state['messages'])} messages. Skipping initial greeting.")
            ...
            await websocket.send_json({"type": "status", "text": "AI is preparing the first question..."})
            
            try:
                # 1. Generate Greeting
                try:
                    agent_result = await asyncio.wait_for(interviewer_node(chat_state), timeout=60.0)
