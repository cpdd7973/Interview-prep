...
class LLMClient:
    ...
    def get_llm(self):
        ...
        if not llm:
            raise RuntimeError(
                "No LLM is available. Please check your GROQ_API_KEY in the .env file."
            )
        return llm
    ...
    async def invoke_async(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            if self.fallback_llm:
                try:
                    response = await asyncio.to_thread(
                        self.fallback_llm.invoke, messages, **kwargs
                    )
                    return response.content
    ...
    def invoke(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        if not llm:
            raise RuntimeError(
                "Primary LLM unavailable and sync invoke doesn't support lazy async fallback."
            )
