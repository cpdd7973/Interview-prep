...
executors = {"default": ThreadPoolExecutor(max_workers=1)}  # Sequential execution only
...
def _run_eval_pipeline_in_background(room_id: str, now: datetime):
    ...
    try:
        logger.info(
            f"🚀 Sweeper: Starting post-interview pipeline for {room_id} in background thread"
        )
        ...
        logger.error(
            f"❌ Sweeper: Evaluation failed for {room_id}: {eval_err}", exc_info=True
        )
        db = SessionLocal()
        session = (
            db.query(InterviewSession)
...
def trigger_evaluation(
    room_id: str, db: Session, session: InterviewSession, now: datetime
):
    ...
    thread = threading.Thread(
        target=_run_eval_pipeline_in_background, args=(room_id, now)
    )
    thread.daemon = True
    thread.start()
...
def state_machine_sweeper():
    ...
    try:
        ...
        pending_sessions = (
            db.query(InterviewSession)
            .filter(InterviewSession.status == SessionStatus.PENDING)
            .all()
        )
        ...
        disconnected_sessions = (
            db.query(InterviewSession)
            .filter(InterviewSession.status == SessionStatus.DISCONNECTED)
            .all()
        )
        ...
        active_sessions = (
            db.query(InterviewSession)
            .filter(InterviewSession.status == SessionStatus.ACTIVE)
            .all()
        )
        ...
        for session in active_sessions:
            ...
            if (
                session.finished_at
                and not session.report_generated_at
