...
def test_multi_turn():
    ...
    if "current_question_id" in res1:
        state["current_question_id"] = res1["current_question_id"]
    if "questions_asked" in res1:
        state["questions_asked"].extend(res1["questions_asked"])
    ...
    state["messages"].append(
        HumanMessage(
            content="A controlled component is one where React state handles the value, while uncontrolled uses refs."
        )
    )
    ...
    if (
        res2.get("current_question_id") != res1.get("current_question_id")
        or res2.get("current_question_id") is None
    ):
